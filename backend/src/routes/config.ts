...
router.get('/', async (req, res) => {
  ...
  if (!rows.length) {
    return res.json({ xtream_server: '', xtream_user: '', xtream_pass: '', xtream_ext: 'ts', plex_server_id: '', app_mappings: {}, xtream_credential_id: null })
  }
...
router.put('/', async (req, res) => {
  ...
  const { xtream_server, xtream_user, xtream_pass, xtream_ext, plex_server_id, app_mappings, xtream_credential_id } = parsed.data
  await db.execute({
    sql: `INSERT INTO device_config (device_id, xtream_server, xtream_user, xtream_pass, xtream_ext, plex_server_id, app_mappings, xtream_credential_id, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    args: [id, xtream_server, xtream_user, xtream_pass, xtream_ext, plex_server_id, JSON.stringify(app_mappings), xtream_credential_id ?? null, Date.now()]
  })
