...
import { ShieldCheck, Plus, Trash2, Pencil, X, Loader2 } from 'lucide-react'
...
const emptyForm: FormState = { name: '', avatar_color: COLORS[0], is_admin: false, pin: '' }
...
export default function Profiles() {
  ...
  const openEdit = (u: User) => {
    setError(null)
    setForm({ id: u.id, name: u.name, avatar_color: u.avatar_color, is_admin: u.is_admin, pin: '' })
  }
        ...
        {users.map(u => (
          <div key={u.id} className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-zinc-500">{u.is_admin ? 'Administrateur' : 'Membre'}</div>
            </div>
